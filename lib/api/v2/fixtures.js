...
import { Balance, BalanceType, Hash, U256, ChannelEntry, PublicKey, stringToU8a, ChannelStatus } from '@hoprnet/hopr-utils';
...
export function channelEntryCreateMock() {
    const pub = PublicKey.from_privkey(stringToU8a('0x1464586aeaea0eb5736884ca1bf42d165fc8e2243b1d917130fb9e321d7a93b8'));
    return new ChannelEntry(pub.clone(), pub, new Balance('1', BalanceType.HOPR), Hash.create([]), U256.one(), U256.one(), ChannelStatus.Closed, U256.one(), U256.one());
}
/**
 ...
 */
export const createTestApiInstance = async (node) => {
 ...
 */
export const createAuthenticatedTestApiInstance = async (node) => {
    const service = express();
    return {
        api: await setupRestApi(service, '/api/v2', node, createTestMocks(), {
            apiToken: 'superuser'
...
export const createMockDb = () => {
    ...
    const db = {
        putSerializedObject: async (ns, id, object) => {
            store[key(ns, id)] = object;
            return;
        },
        getSerializedObject: async (ns, id) => {
            return store[key(ns, id)];
        },
        deleteObject: async (ns, id) => {
