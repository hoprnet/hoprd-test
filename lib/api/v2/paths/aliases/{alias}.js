 ...
 * @returns new state
 */
export const removeAlias = (stateOps, alias) => {
 ...
 */
export const getAlias = (state, alias) => {
    const peerId = state.aliases.get(alias);
    if (!peerId)
        throw Error(STATUS_CODES.PEERID_NOT_FOUND);
...
const GET = [
    async (req, res, _next) => {
        ...
        try {
            const peerId = getAlias(stateOps.getState(), alias);
...
//# sourceMappingURL=%7Balias%7D.js.map