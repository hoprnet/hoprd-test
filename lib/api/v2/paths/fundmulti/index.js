import { defer, generate_channel_id, PublicKey } from '@hoprnet/hopr-utils';
import { peerIdFromString } from '@libp2p/peer-id';
import BN from 'bn.js';
import { STATUS_CODES } from '../../utils.js';
const fundingRequests = new Map();
async function validateFundChannelMultiParameters(node, counterpartyStr, outgoingAmountStr, incomingAmountStr) {
    let counterparty;
    try {
 ...
 */
export async function fundMultiChannels(node, counterpartyStr, outgoingAmountStr, incomingAmountStr) {
    ...
    const outgoingChannelId = generate_channel_id(node.getEthereumAddress(), PublicKey.from_peerid_str(validationResult.counterparty.toString()).to_address());
    const incomingChannelId = generate_channel_id(PublicKey.from_peerid_str(validationResult.counterparty.toString()).to_address(), node.getEthereumAddress());
    ...
    if (fundingOutgoingChannelRequest == null && fundingIncomingChannelRequest == null) {
        ...
        fundingOutgoingChannelRequest = defer();
        fundingIncomingChannelRequest = defer();
    ...
    try {
        const receipt = await node.fundChannel(validationResult.counterparty, validationResult.outgoingAmount, validationResult.incomingAmount);
        return {
            success: true,
                ...
                schema: {
                    ...
                    properties: {
                        ...
                        outgoingAmount: {
                            format: 'amount',
                            type: 'string',
                            description: 'Amount of HOPR tokens to fund the outgoing channel (node -> counterparty). It will be used to pay for sending messages through channel'
                        },
                        incomingAmount: {
                            format: 'amount',
                            type: 'string',
                            description: 'Amount of HOPR tokens to fund the incoming channel (counterparty -> node). It will be used to pay for sending messages through channel'
                        }
                    },
                    example: {
                    ...
                    schema: {
                        ...
                        properties: {
                            receipt: {
                                ...
                                description: 'Receipt for fund multi channels transaction. Can be used to check status of the smart contract call on blockchain.'
                            }
                        }
                    }
                }
