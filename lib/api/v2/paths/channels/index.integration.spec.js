...
import { createTestApiInstance, ALICE_PEER_ID, BOB_PEER_ID, CHARLIE_PEER_ID, ALICE_NATIVE_ADDR, INVALID_PEER_ID, channelEntryCreateMock } from '../../fixtures.js';
...
node.openChannel = sinon.fake.returns(Promise.resolve({
    channelId: CHANNEL_ID,
    receipt: 'testReceipt'
}));
describe('GET /channels', function () {
    const incoming = new ChannelEntry(PublicKey.from_peerid_str(ALICE_PEER_ID.toString()), PublicKey.from_peerid_str(BOB_PEER_ID.toString()), new Balance('1', BalanceType.HOPR), Hash.create([]), U256.one(), U256.one(), ChannelStatus.Closed, U256.one(), U256.one());
    const outgoing = new ChannelEntry(PublicKey.from_peerid_str(BOB_PEER_ID.toString()), PublicKey.from_peerid_str(ALICE_PEER_ID.toString()), new Balance('2', BalanceType.HOPR), Hash.create([]), new U256('2'), new U256('2'), ChannelStatus.Closed, new U256('2'), new U256('2'));
    const otherChannel = new ChannelEntry(PublicKey.from_peerid_str(BOB_PEER_ID.toString()), PublicKey.from_peerid_str(CHARLIE_PEER_ID.toString()), new Balance('3', BalanceType.HOPR), Hash.create([]), new U256('3'), new U256('3'), ChannelStatus.WaitingForCommitment, new U256('3'), new U256('3'));
