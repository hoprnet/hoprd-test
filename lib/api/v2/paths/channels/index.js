import { ChannelStatus, defer, generate_channel_id, PublicKey, channel_status_to_string } from '@hoprnet/hopr-utils';
 ...
 */
export const formatChannelTopologyInfo = (channel) => {
    return {
...
export const formatOutgoingChannel = (channel) => {
    return {
        type: 'outgoing',
        channelId: channel.get_id().to_hex(),
...
export const formatIncomingChannel = (channel) => {
    return {
        type: 'incoming',
        channelId: channel.get_id().to_hex(),
 ...
 */
export const getAllChannels = async (node) => {
    const channels = await node.getAllChannels();
    const channelTopology = channels.map(formatChannelTopologyInfo);
...
async function validateOpenChannelParameters(node, counterpartyStr, amountStr) {
    let counterparty;
    try {
 ...
 */
export async function openChannel(node, counterpartyStr, amountStr) {
    const validationResult = await validateOpenChannelParameters(node, counterpartyStr, amountStr);
    if (validationResult.valid == false) {
        return { success: false, reason: validationResult.reason };
    }
    const channelId = generate_channel_id(node.getEthereumAddress(), PublicKey.from_peerid_str(validationResult.counterparty.toString()).to_address());
    let openingRequest = openingRequests.get(channelId.to_hex());
    if (openingRequest == null) {
        openingRequest = defer();
