...
const hops = parseInt(process.env.K6_HOPS || '1', 10);
...
Promise.all(hoprdNodes).then((hoprdNodes: HoprdNode[]) => {
  setupEnvironment(hoprdNodes).then(() => {
    ...
    const k6TestRunTemplateParsed = k6TestRunTemplate({ testName, clusterNodes, topologyName, workloadName, hops, requestsPerSecondPerVu, testid, duration, iterationTimeout, vuPerRoute });
    ...
    const k6TestResultsTemplateParsed = k6TestResultsTemplate({ testName, clusterNodes, topologyName, workloadName, hops, requestsPerSecondPerVu, testid, duration, iterationTimeout });
