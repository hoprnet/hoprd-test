...
const amountOfSenders = nodesData.nodes.filter(
  (node: any) => node.enabled && node.isSender != undefined && node.isSender,
).length;
const amountOfReceivers = nodesData.nodes.filter(
  (node: any) =>
...
for (scenario in optionsData.scenarios) {
  if (scenario !== "receive_messages") {
    if (scenario !== "incremental" && scenario !== "hysteresis") {
      // Incremental and Hysteresis scenarios should keep stage 0 target as is
      optionsData.scenarios[scenario].stages[0].target =
        amountOfSenders *
        (__ENV.SCENARIO_ITERATIONS ||
          optionsData.scenarios[scenario].stages[0].target);
    }
    optionsData.scenarios[scenario].stages[1].target =
      amountOfSenders *
      (__ENV.SCENARIO_ITERATIONS ||
        optionsData.scenarios[scenario].stages[1].target);
    if (__ENV.SCENARIO_DURATION) {
      if (scenario === "hysteresis") {
        // Hysteresis scenario should divide the duration in half for stage 1 and 2
        ...
        optionsData.scenarios[scenario].stages[1].duration =
          __ENV.SCENARIO_DURATION + "m";
      }
    }
    optionsData.scenarios[scenario].preAllocatedVUs = Math.max(
      1,
      Math.floor(
        (amountOfSenders *
          (Number(__ENV.SCENARIO_ITERATIONS) ||
            optionsData.scenarios[scenario].stages[1].target)) /
          2,
      ),
    );
  ...
  if (execution.vu.idInInstance <= amountOfReceivers) {
    ...
    const websocketResponse = ws.connect(
      ...
      function (socket) {
        socket.on("open", () =>
          console.log(
            `Connected via websocket to node ${receiverHoprdNode.name}`,
          ),
        );
        socket.on("message", (data) => {
          ...
          if (messageType === "message") {
            ...
            console.log(
              `Message received on ${receiverHoprdNode.name} relayed from ${tags.path} with latency ${duration} ms`,
            );
            ...
            console.log(
              `Unknown message type ${messageType} received on ${receiverHoprdNode.name}`,
            );
          }
        });
        socket.on("error", (error) => {
          ...
          console.error(
            `Node ${receiverHoprdNode.name} replied with a websocket error:`,
            error,
          );
        });
        socket.on("close", (errorCode: any) => {
          __ENV.WEBSOCKET_DISCONNECTED = "true";
          console.log(
            `Disconnected via websocket from node ${receiverHoprdNode.name} due to error code ${errorCode} at ${new Date().toISOString()}`,
          );
        });
      },
    );
...
// This function is executed for each iteration
// default function imports the return data from the setup function https://docs.k6.io/docs/test-life-cycle
export function multipleHopMessage(dataPool: {
  senders: HoprdNode[];
  relayers: HoprdNode[];
  receivers: HoprdNode[];
}) {
  const hops = Number(__ENV.HOPS) || 1;
  const nodeIndex = Math.ceil(
    execution.vu.idInInstance % (amountOfSenders * scenariosLength),
  );
  ...
  const receivers = dataPool.receivers.filter(
    (receiver: HoprdNode) => receiver.name != sender.name,
  );
  let receiver;
  if (receivers.length === 0) {
    fail(
      `No receiver nodes available for this VU '${execution.vu.idInInstance}'`,
    );
  } else {
  ...
  const relayers = dataPool.relayers.filter(
    (relayer: HoprdNode) =>
      relayer.name != sender.name && relayer.name != receiver.name,
  );
  ...
  sendMessage(
    ...
    sender.httpParams,
    JSON.stringify({
      tag: execution.vu.idInInstance + 1024,
      body,
      path,
...
export function teardown(dataPool: {
  ...
  console.log(
    "teardown will still be called even when calling exec.test.abort()",
  );
}
