...
export function setup() {
  ...
  return routes.map((route) => {
    let downloadSession = route.sender.openSession(route.relayer, route.receiver, "udp", configuration.getTargetDestination('download'));

    //console.log(`[Download][VU ${vu +1}] Opening download udp connection to ${listenHost}`)
    let downloadConnection = udp.connect(downloadSession);
...
export function download(routes: [{ sender: HoprdNode, relayer: HoprdNode, receiver: HoprdNode, downloadSession: string, downloadConnection: object }]) {
  ...
  const connection = routes[vu].downloadConnection;
